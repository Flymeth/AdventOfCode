const { hands, validCardsPart2: validCards } = require("./module")
...
function getHandType(hand) {
    ...
    let bonus = 0
    if(cards.includes("J")) {
        bonus = hand.match(new RegExp("J", "g")).length
        cards.splice(cards.indexOf("J"), 1)
    }

    const cardsNumbers = cards.map(card => hand.match(new RegExp(card, "g")).length + bonus)
    const maxSameCardAmount = Math.max(...cardsNumbers)

    if(maxSameCardAmount === 5 || !cards.length) return 7
